...
[pg, ~, rfg]=VW_gre6(t,nlay,q,rw,dfw,phi,h,ks,k,rs,re,etaw,etao,lw,lo,fcce);

% calculando os dados de pressao e vazao
[pa, ~, rfa]=VW_p(t,nlay,q,rw,dfw,phi,h,ks,k,rs,re,etaw,etao,lw,lo,fcce);
...
%             rf=rf(1);
            if rf<rs
                r1=rf; r2=rs; eta2=etao./k.*ks;   l2=lo./k.*ks;
            else
                r1=rs; r2=rf; eta2=etaw.*k./ks;   l2=lw./ks.*k;
            end
            ...
            for ia=2:ii
                ...
                for kk=1:N
                    ...
                    x1=Vj(N,kk)*sisgre(u1,1,h(jj),rw,r1,r2,re,lw,l2,lo,etaw,eta2,etao,0,fcce);
                    x2=Vj(N,kk)*sisgre(u2,1,h(jj),rw,r1,r2,re,lw,l2,lo,etaw,eta2,etao,0,fcce);
                    x3=Vj(N,kk)*sisgre(u3,1,h(jj),rw,r1,r2,re,lw,l2,lo,etaw,eta2,etao,0,fcce);
                    % incrementando a soma
                    imp1=imp1+x1(1)*besseli(0,sqrt(rw*rw*u1/etaw));
                    imp1=imp1+x1(2)*besselk(0,sqrt(rw*rw*u1/etaw));
                    
                    imp2=imp2+x2(1)*besseli(0,sqrt(rw*rw*u2/etaw));
                    imp2=imp2+x2(2)*besselk(0,sqrt(rw*rw*u2/etaw));
                    
                    imp3=imp3+x3(1)*besseli(0,sqrt(rw*rw*u3/etaw));
                    imp3=imp3+x3(2)*besselk(0,sqrt(rw*rw*u3/etaw));
                    switch fcce
                        % somando os pocos imagem para canal
                        case 3
                            for n=1:infi
                                imp1=imp1+2*x1(5)*besselk(0,2*n*re*sqrt(u1/etao));
                                imp2=imp2+2*x2(5)*besselk(0,2*n*re*sqrt(u2/etao));
                                imp3=imp3+2*x3(5)*besselk(0,2*n*re*sqrt(u3/etao));
                            end
                            % somando o poco imagem para falha selante
                        case 4
                            imp1=imp1+x1(5)*besselk(0,2*re*sqrt(u1/etao));
                            imp2=imp2+x2(5)*besselk(0,2*re*sqrt(u2/etao));
                            imp3=imp3+x3(5)*besselk(0,2*re*sqrt(u3/etao));
...
function [pwf, qj, rf]=VW_p(t,nlay,q,rw,dfw,phi,h,ks,k,rs,re,etaw,etao,lw,lo,fcce)
    ...
    for jj=1:nlay
        ...
        for ii=1:dim
            ...
            for kk=1:N
                ...
                u=log(2)*kk/t(ii);
                % calculando o valor de x
                x=Vj(N,kk)*sis(u,q,h(jj),rw,r1,r2,re,lw,l2,lo,etaw,eta2,etao,fcce);
                % incrementando a soma
                pwf(ii)=pwf(ii)+x(1)*besseli(0,sqrt(rw*rw*u/etaw))+...
                    x(2)*besselk(0,sqrt(rw*rw*u/etaw));
                switch fcce
                    % somando os pocos imagem para canal
                    case 3
                        for n=1:infi
                            pwf(ii)=pwf(ii)+2*x(5)*besselk(0,2*n*re*sqrt(u/etao));
                        end
                % somando o poco imagem para falha selante
                    case 4
                    pwf(ii)=pwf(ii)+x(5)*besselk(0,2*re*sqrt(u/etao));
                % somando o poco imagem para aquifero lateral
                    case 5
                    pwf(ii)=pwf(ii)-x(5)*besselk(0,2*re*sqrt(u/etao));
                end
