...
[pp, qp, rfp]=HW_per(t,nlay,q,rw,dfp,phi,ct,h,len,ks,k,rs,etaw,etao,ltp,lo/k);

[pd, qd]=HW_dav(t,nlay,q,rw,phi,ct,h,len,ks,k,rs,etao,lo/k);

% calculando os dados de pressao e vazao
[pg, ~, rfg]=HW_gre2(t,nlay,q,rw,dfg,phi,h,len,ks,k,rs,etaw,etao,lw,lo);
...
% pegando so os dados a partir de t = 9.6e-5
jj=find(abs(t-9.6e-5)<1e-10);
pg=pg(jj:end);      pp=pp(jj:end);      pd=pd(jj:end);
dpg=dpg(jj:end);	dpp=dpp(jj:end);	dpd=dpd(jj:end);
...
function [pwf,qj,rf]=HW_gre2(t,nlay,q,rw,dfw,phi,h,len,ks,k,rs,etaw,etao,lw,lo)
...
%     end
    ...
    dy=0.732*len;
    iy=erf(dy./2./sqrt(etao.*t))/2-erf(-dy./2./sqrt(etao.*t))/2;
...
function [pwf, qj, rf]=HW_per(t,nlay,q,rw,dfw,phi,ct,h,len,ks,k,rs,etaw,etao,lt,lo)
    iaux=find(t>1e-5); iaux=iaux(1);
%     iaux=2;
    ...
    dps=q/k/lo/len*(k/ks-1)*log(rs/rw);
...
%     aj=ones(dim,nlay);
    ...
    for ii=1:length(t)
        inst(ii)=HW_inst_sourc(t(ii),etao,k,k,k,k,len,dx,dy,dz,h);
    end
    ...
    for ii=iaux:dim
        ...
        dpl=q*calc_aj(rw,k,len,ks,rs,rf,lo,lt);
...
%% funcao que calcula a pressao em HW 1 camada ============================
function [pwf, qj]=HW_dav(t,nlay,q,rw,phi,ct,h,len,ks,k,rs,etao,lo)
    iaux=find(t>1e-5); iaux=iaux(1);
%     iaux=2;
...
function [sources] = HW_inst_sourc(t,eta,kx,ky,kz,k,lenj,dx,dy,dz,h)
